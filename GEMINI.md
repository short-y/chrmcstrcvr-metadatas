...
*   **`play_radio_stream_v2.py` / `play_kozt.py` Behavior**: Scripts pre-fetch metadata before starting playback.
*   **Logging**: Use multi-level logging conventions:
    *   **Default**: `WARNING` level (Minimal output).
    *   **`--debug`**: `INFO` level (Heartbeats, status updates).
    *   **`--verbose`**: `DEBUG` level (Full internal logs).
